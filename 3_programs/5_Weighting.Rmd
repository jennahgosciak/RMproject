...
merged_oz_wide <- merged_oz %>% 
  filter(year < 2018 & year > 2014) %>% 
  pivot_wider(id_cols = c("GEOID"), names_from = c("year"), values_from = pct_outcomes) %>% 
  left_join(merged_oz %>% 
              filter(year == 2014) %>% 
              select(GEOID, cov, treatment), by = "GEOID")
merged_oz_wide
```

```{r}
pct_outcomes_yr <- str_c(pct_outcomes, c(2015:2017), sep = "_")
covariates <- str_c(c(cov, pct_outcomes_yr), collapse = " + ")
form <- formula(paste0("treatment ~", covariates))
m.out0 <- matchit(form, data = merged_oz_wide,
...
```{r}
# matching
m.out1 <- matchit(form, data = merged_oz_wide,
...
plot(m.out1, type = "density", interactive = FALSE,
     which.xs = c(cov, pct_outcomes_yr))
```
```{r}
plot(summary(m.out1))
```
