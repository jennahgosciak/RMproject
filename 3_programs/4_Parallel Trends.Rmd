...
event_model <- function(outcome, data, covariates) {
  form <- paste0(outcome, " ~ treatment:year_fact2015 + ",
                    "treatment:year_fact2016 + treatment:year_fact2018 + treatment:year_fact2019 + ",
                    "year_fact2015 + year_fact2016 + year_fact2018 + year_fact2019")
...
event_df <- bind_rows(map_dfr(pct_outcomes, ~generate_event_df(., matched_data, covariates = F)),
                      map_dfr(pct_outcomes, ~generate_event_df(., matched_data, covariates = T)))

event_df
```

```{r, fig.height = 15, fig.width = 5}
generate_event_plot <- function(df, type) {
  df %>% 
  ...
  ggsave(paste0("../2_output/", type, "event_study.png"),
         height = 15)
}

generate_event_plot(event_df, "rac_")
generate_event_plot(event_df, "wac_")
generate_event_plot(event_df, "S000_")
```
