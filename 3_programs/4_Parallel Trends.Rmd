...
cov <- c("log_median_household_income", "total_housing", "percent_white", "percent_postsec", "percent_rent", "percent_hc_covered", "percent_poverty", "percent_suplemental_income", "percent_employed", "dec_score", "vacancyrate", "pctunder18", "pctover64")
...
```{r}
event_years <- c("yearmin3", "yearmin2", "year0", "year1")
...
event_model <- function(outcome, data, covariates) {
  form <- paste0(outcome, " ~ yearmin3 + yearmin2 + ",
                 "year0 + year1")
...
coef_vals[,] %>% 
  ...
  bind_rows(tibble(year = "yearmin1", Estimate = 0.00, estimate_ci_high=0,
                   estimate_ci_low=0)) %>% 
  mutate(period = c(-3, -2, -1, 0, 1)[match(year, c("yearmin3",
                                                    "yearmin2", "yearmin1", 
                                                    "year0", "year1"))],
         outcome = outcome,
...
event_df_pre <- topcommutezones %>%
  filter(year > 2014) %>%
  mutate(yearmin3 = if_else(treatment == 1 & year == 2015, 1, 0),
         yearmin2 = if_else(treatment == 1 & year == 2017, 1, 0),
         yearmin1 = if_else(treatment == 1 & year == 2016,1, 0),
         year0 = if_else(treatment == 1 & year == 2018,1, 0),
         year1 = if_else(treatment == 1 & year == 2019,1,0))
