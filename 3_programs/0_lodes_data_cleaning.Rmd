  ...
  if (type == "wac" | type == "rac") {
    ...
    df_oz <- df_oz %>% 
      left_join(oz %>% 
                  ...
                  rename(h_desig = DESIGNATED),
                by = c("w_tract" = "GEOID")) %>% 
      mutate(same_tract = if_else(GEOID == w_tract & !is.na(w_tract), 1, 0)) %>% 
      mutate(oz_desig_tract = if_else(h_desig %in% c(TRUE), 1, 0)) %>% 
      mutate(oz_elig_tract = if_else(h_desig %in% c(FALSE), 1, 0)) %>% 
      group_by(GEOID, LIC, CONTIGUOUS, CROSS_STATE, designated_desc,
               year, same_tract, oz_desig_tract, oz_elig_tract) %>% 
...
plot_df <- function(df, column, desc) {
  ...
  print(plot)
  ggsave(str_c("../3_output/", desc, ".png"))
}
...
```{r}
# load residence area characteristics
od_oz_14_r <- map_dfr("ca", ~get_lodes_yr(., 2014, oz, "od", "h_tract"))
od_oz_14_r %>% 
      write_csv("od_oz_14_r.csv")
```
```{r}
od_oz_15_r <- map_dfr(states, ~get_lodes_yr(., 2015, oz, "od", "h_tract"))
od_oz_16_r <- map_dfr(states, ~get_lodes_yr(., 2016, oz, "od", "h_tract"))
od_oz_17_r <- map_dfr(states, ~get_lodes_yr(., 2017, oz, "od", "h_tract"))
od_oz_18_r <- map_dfr(states, ~get_lodes_yr(., 2018, oz, "od", "h_tract"))
od_oz_19_r <- map_dfr(states, ~get_lodes_yr(., 2019, oz, "od", "h_tract"))
```
