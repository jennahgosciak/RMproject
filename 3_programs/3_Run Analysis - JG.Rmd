...
run_model_outcome <- function(outcome, data=merged_oz, covariates=cov, print=TRUE) {
  ...
  coef <- round(sum_fit_plm$coefficients[, 1], 3)
  pval <- round(sum_fit_plm$coefficients[, 4], 4)
  ...
  results <- data.frame("outcome" = outcome,
               ...
               "pval" = sum_fit_plm$coefficients[, 4],
...
run_model_outcome("S000_same_tract", print=FALSE)
```
```{r}
results <- map_dfr(c("C000rac", "C000wac",
       "CA01rac","CA01wac",
       "CE01rac", "CE01wac",
       "S000_same_tract", "S000_desig", "S000_elig"), ~run_model_outcome(., print=FALSE))
