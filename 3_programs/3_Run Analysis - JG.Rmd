...
merged_oz <- readRDS("../2_output/analysis_file.RDS") %>% 
  ...
  mutate(across(all_of(outcomes), .fns = list(log = ~log(if_else(. == 0, 1, .))), .names = "{.col}_{.fn}")) %>% 
  # create alternate treatment for a placebo test
  mutate(treatment2 = if_else(year >= 2015, 1, 0)) %>% 
  mutate(post_treatment = post*treatment,
         post_treatment2 = post*treatment2) %>% 
  mutate(poverty_rate = povertystatus / povertystatus_denom) %>% 
  filter(!is.na(GEOID) & !is.na(year)) %>% 
  filter((LIC == TRUE & DESIGNATED == FALSE) | DESIGNATED == TRUE)
...
twfe_model <- function(outcome, data, covariates) {
  return(plm(paste0(outcome, " ~ post_treatment + year_fact"), 
               data = data, 
               index = c("GEOID"), 
               model = "within", 
               effect = "individual"))
...
twfe_model_covariates <- function(outcome, data, covariates) {
  covariates_list <- str_c(covariates, ":year_fact") %>% 
    str_c(collapse = " + ")
  return(plm(paste0(outcome, " ~ post_treatment + year_fact + ", covariates_list), 
               data = data, 
               index = c("GEOID"), 
               model = "within", 
               effect = "individual"))
...
generate_model_output <- function(fit_model, outcome, treat_var, model_type, data, print) {
  ...
  if (model_type %in% c("twfe", "twfe_cov")) {
...
models <- c("twfe_cov", "twfe", "did", "time_trend")
run_model_outcome("S000_same_tract_log", models,  print=T)
```
```{r}
