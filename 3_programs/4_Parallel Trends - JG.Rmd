...
merged_oz <- readRDS("../2_output/analysis_file.RDS") %>% 
  ...
  mutate(across(C000rac:CRothwac, .fns = list(log = ~log(if_else(. == 0, 1, .))), .names = "{.col}_{.fn}")) %>% 
...
# balance test
balance_tests <- map_dfr(c("poverty_rate", "medfamincome"), ~run_balance_tests(., merged_oz %>% 
                                                                                 filter(year == 2017)))
```
